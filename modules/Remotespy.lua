...
function remoteclass.new(remote, method, args, returnedvalue, callingscript, callingfunction)
	local class = {}
	class.remote, class.method, class.args, class.returnedvalue, class.callingscript, class.callingfunction =
		remote, method, args, returnedvalue, callingscript, callingfunction
...
old = hookmetamethod(
	...
	newcclosure(function(...)
		...
		if
			...
			and (string.gsub(method, "^%l", string.upper) == "FireServer" or method == "InvokeServer" or method == "Fire" or method == "Invoke")
			and (
				self.ClassName and self.ClassName == "RemoteEvent"
				or self.ClassName == "RemoteFunction"
				or self.ClassName == "BindableEvent"
				or self.ClassName == "BindableFunction"
			)
		then
			local oldid = getthreadidentity()
			setthreadidentity(8)
			if
				getgenv().loggedremotes.blockedremotes["All"][GetDebugId(self) .. method]
				or (
					getgenv().loggedremotes.blockedremotes["Args"][(GetDebugId(self)) .. method]
					and comparetables(
						getgenv().loggedremotes.blockedremotes["Args"][(GetDebugId(self)) .. method].args,
						args
					)
				)
			then
				return
			elseif
				getgenv().loggedremotes.ignoredremotes["All"][(GetDebugId(self)) .. method]
				or (getgenv().loggedremotes.ignoredremotes["Args"][(GetDebugId(self)) .. method] and comparetables(
					getgenv().loggedremotes.ignoredremotes["Args"][(GetDebugId(self)) .. method].args,
					args
				))
				or getgenv().iscaller and caller
			then
				return old(...)
			end
			local returnedvalue = old(...)
			local remote =
				remoteclass.new(cloneref(self), method, args, returnedvalue, callingscript, debug.info(3, "f"))
...
for i, v in pairs(getinstances()) do
	if v:IsA("BaseRemoteEvent") then
		...
		v.OnClientEvent:Connect(function(...)
			...
			if
				getgenv().loggedremotes.blockedremotes["All"][GetDebugId(v) .. method]
				or (
					getgenv().loggedremotes.blockedremotes["Args"][(GetDebugId(v)) .. method]
					and comparetables(
						getgenv().loggedremotes.blockedremotes["Args"][(GetDebugId(v)) .. method].args,
						args
					)
				)
			then
				return
			elseif
				getgenv().loggedremotes.ignoredremotes["All"][(GetDebugId(v)) .. method]
				or (getgenv().loggedremotes.ignoredremotes["Args"][(GetDebugId(v)) .. method] and comparetables(
					getgenv().loggedremotes.ignoredremotes["Args"][(GetDebugId(v)) .. method].args,
					args
				))
				or getgenv().iscaller and caller
			then
	...
	elseif v:IsA("RemoteFunction") then
		if getcallbackvalue and pcall(getcallbackvalue, v, "OnClientInvoke") then
			...
			local _, old = pcall(
				hookfunction,
				getcallbackvalue(v, "OnClientInvoke"),
				newcclosure(function(...)
					local oldid = getthreadidentity()
					setthreadidentity(8)
					local addcall = loadstring(
						game:HttpGet(
							"https://raw.githubusercontent.com/0Void2391/Sulfoxide/refs/heads/main/ui/modules/Remotespy.lua"
						)
					)() --upvalue exceeded fix
...
old = hookfunction(
	...
	newcclosure(function(...)
		...
		elseif
			getgenv().loggedremotes.ignoredremotes["All"][(GetDebugId(self)) .. method]
			or (getgenv().loggedremotes.ignoredremotes["Args"][(self.GetDebugId(self)) .. method] and comparetables(
				getgenv().loggedremotes.ignoredremotes["Args"][(GetDebugId(self)) .. method].args,
				args
			))
			or getgenv().iscaller and caller
